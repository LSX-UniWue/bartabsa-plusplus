...
class ABSATokenizer(BaseMappingTokenizer):
    def __init__(
        self,
    ...
    def pointer_generator(
        ...
        for _, (s_start, s_end, t_start, t_end, sentiment) in enumerate(labels):
            ...
            pointer_labels = pointer_labels + [
                word_mapping[s_start][0] + offset,
                word_mapping[s_end][-1] + offset,
                word_mapping[t_start][0] + offset,
                word_mapping[t_end][-1] + offset,
                self.mapping2targetID[sentiment] + 1,
            ]

            output_encoding.extend([input_encoding_ids[i - offset] for i in pointer_labels[-5:-1]] + [self.mapping2ID[sentiment]])
