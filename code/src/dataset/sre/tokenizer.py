...
class SRETokenizer(BaseMappingTokenizer):
    def __init__(
        self,
    ...
    def pointer_generator(
        ...
        for _, (src_start, src_end, tgt_start, tgt_end, relation) in enumerate(labels):
            ...
            pointer_labels = pointer_labels + [
                word_mapping[src_start][0] + offset,
                word_mapping[src_end][-1] + offset,
                word_mapping[tgt_start][0] + offset,
                word_mapping[tgt_end][-1] + offset,
                self.mapping2targetID[relation] + 1,
            ]

            output_encoding.extend([input_encoding_ids[i - offset] for i in pointer_labels[-5:-1]] + [self.mapping2ID[relation]])
